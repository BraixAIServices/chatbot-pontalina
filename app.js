...
module.exports = function(app) {
  ...
  middleware.after = function(message, assistantResponse, callback) {
    ...
    //console.log('Usuário: '+ message.address.user.name);

    if(assistantResponse.intents.length > 0 ){
      console.log('Intenção detectada: '+ assistantResponse.intents[0].intent);
      console.log('Usuário: '+ message.address.user.name);
      if(assistantResponse.intents[0].intent == 'despedida'){
          console.log("Fim");
      }
    }
